...
while j<loop:
    ...
    bornesup=zeta/(1.-np.exp(zeta*(beta_min-beta_max)))

    borneinf=zeta/(np.exp(zeta*(beta_max-beta_min))-1.)

    pebble=np.random.uniform(borneinf,bornesup)

    beta=-1./zeta*np.log(pebble*(np.exp(-zeta*beta_min)-np.exp(-zeta*beta_max)))
